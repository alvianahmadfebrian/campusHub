...
class DriveController extends Controller
{
    public function index(Request $request, ?string $folderId = null): Response
    {
        ...
        $folder = $folderId ? $this->ownedFolder($folderId, $userId) : null;
        ...
        $folders = DB::table('drive_folders')
            ...
            ->orderBy('nama')
            ->get()
            ->map(fn ($item) => $this->folderResource($item));
        ...
        $files = DB::table('drive_files')
            ...
            ->get()
            ->map(fn ($item) => $this->fileResource($item));

        return Inertia::render('Drive/Index', [
            'currentFolder' => $folder ? $this->folderResource($folder) : null,
            'breadcrumbs' => $folder ? $this->breadcrumbs($folder, $userId) : [],
            'folders' => $folders,
            'files' => $files,
            'limits' => [
                'maxUploadMb' => 50,
    ...
    {
        $folder = $this->ownedFolder($id, $this->userId($request));
        $validated = $request->validate([
        ...
        DB::table('drive_folders')->where('id', $folder->id)->update($update);

        return back()->with('success', 'Folder berhasil diperbarui.');
    }

    public function destroyFolder(Request $request, string $id, SupabaseStorage $storage): RedirectResponse
    {
        $userId = $this->userId($request);
        ...
        DB::table('drive_folders')->where('id', $folder->id)->delete();
    ...
    public function storeFile(Request $request, SupabaseStorage $storage): RedirectResponse
    {
        $userId = $this->userId($request);
        $validated = $request->validate([
        ...
        $storageFolder = 'drive/' . $userId . ($folderId ? '/' . $folderId : '/root');

        try {
            $path = $storage->put($this->driveBucket(), $uploadedFile, $storageFolder);
        } catch (Throwable $exception) {
            return back()->withErrors([
    ...
    public function updateFile(Request $request, string $id): RedirectResponse
    {
        ...
        DB::table('drive_files')->where('id', $file->id)->update($update);

        return back()->with('success', 'File berhasil diperbarui.');
    }

    public function destroyFile(Request $request, string $id, SupabaseStorage $storage): RedirectResponse
    {
        $file = $this->ownedFile($id, $this->userId($request));
        ...
        DB::table('drive_files')->where('id', $file->id)->delete();

        return back()->with('success', 'File berhasil dihapus.');
    }

    public function downloadFile(Request $request, string $id, SupabaseStorage $storage): StreamedResponse
    {
        $file = $this->ownedFile($id, $this->userId($request));
     ...
     * Berbagi folder membuka seluruh isi turunannya, seperti tautan folder Drive.
     */
    public function publicFolder(string $token, ?string $folderId = null): Response
    {
        ...
        $current = $folderId ? DB::table('drive_folders')->where('id', $folderId)->first() : $root;
        abort_unless($current && $this->isFolderInside($root->id, $current->id, $root->owner_id), 404);
        ...
        $folders = DB::table('drive_folders')
            ...
            ->map(fn ($folder) => [
                'id' => $folder->id,
                'nama' => $folder->nama,
                'url' => route('share.folder', ['token' => $token, 'folderId' => $folder->id]),
            ]);
        ...
        $files = DB::table('drive_files')
            ...
            ->map(fn ($file) => [
                ...
                'url' => route('share.folder.file', ['token' => $token, 'fileId' => $file->id]),
            ]);

        return Inertia::render('Drive/PublicFolder', [
            'rootFolder' => ['nama' => $root->nama],
            'currentFolder' => ['id' => $current->id, 'nama' => $current->nama],
    ...
    public function publicFolderFile(string $token, string $fileId, SupabaseStorage $storage): StreamedResponse
    {
        $root = DB::table('drive_folders')
            ->where('share_token', $token)
            ->where('is_public', true)
        ...
        abort_unless($file && $file->folder_id && $this->isFolderInside($root->id, $file->folder_id, $root->owner_id), 404);

        return $this->serveFile($file, $storage, true);
    }

    public function publicFile(string $token, SupabaseStorage $storage): StreamedResponse
    {
        $file = DB::table('drive_files')
            ->where('share_token', $token)
            ->where('is_public', true)
    ...
    private function serveFile(object $file, SupabaseStorage $storage, bool $inline): StreamedResponse
    {
        $stream = $storage->readStream($this->driveBucket(), $file->storage_path);
        $disposition = ResponseHeaderBag::makeDisposition(
            $inline ? ResponseHeaderBag::DISPOSITION_INLINE : ResponseHeaderBag::DISPOSITION_ATTACHMENT,
            $file->nama_tampilan,
            Str::ascii($file->nama_tampilan) ?: 'download'
        );
    ...
    private function folderResource(object $folder): array
    {
        return [
            ...
            'open_url' => route('drive.index', ['folderId' => $folder->id]),
            'share_url' => $folder->is_public ? route('share.folder', ['token' => $folder->share_token]) : null,
        ];
    }
    ...
    private function fileResource(object $file): array
    {
        return [
            ...
            'download_url' => route('drive.files.download', ['id' => $file->id]),
            'share_url' => $file->is_public ? route('share.file', ['token' => $file->share_token]) : null,
        ];
    }
    ...
    private function breadcrumbs(object $folder, string $userId): array
    {
        ...
        while ($current && $guard < 100) {
            array_unshift($result, [
                'nama' => $current->nama,
                'url' => route('drive.index', ['folderId' => $current->id]),
            ]);

            $current = $current->parent_id
                ? DB::table('drive_folders')->where('owner_id', $userId)->where('id', $current->parent_id)->first()
                : null;
            $guard++;
        }
    ...
    private function isFolderInside(string $rootId, string $folderId, string $ownerId): bool
    {
        $currentId = $folderId;
        $guard = 0;
    ...
    private function publicBreadcrumbs(object $root, object $current, string $token): array
    {
        $result = [];
        $cursor = $current;
        $guard = 0;
        ...
        while ($cursor && $guard < 100) {
            array_unshift($result, [
                'nama' => $cursor->nama,
                'url' => route('share.folder', ['token' => $token, 'folderId' => $cursor->id === $root->id ? null : $cursor->id]),
            ]);
            ...
            $cursor = $cursor->parent_id
                ? DB::table('drive_folders')->where('owner_id', $root->owner_id)->where('id', $cursor->parent_id)->first()
                : null;
            $guard++;
        }
    ...
    {
        return (string) config('services.supabase.storage.buckets.drive', 'drive-files');
    }
}
